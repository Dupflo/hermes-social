...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    # A pure-password provider (e.g. the built-in "basic" username/password
    # provider) has no OAuth redirect flow — its start_login() raises
    # NotImplementedError, so redirecting to /auth/login?provider=basic 500s
    # on first touch. Auto-SSO only applies to OAuth redirect providers; fall
    # through to the /login credential form for password providers.
